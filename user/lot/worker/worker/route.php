...
Route::set($path, function() use($path, $language, $site) {
    ...
    if ($r = Request::post()) {
        ...
        if ($is_enter) {
            // Check token…
            if (empty($token) || !Guardian::check($token)) {
                Message::error('token');
            } else {
                File::open(USER . DS . $key . DS . 'token.data')->delete();
            ...
            // No error(s), go to the next step(s)…
                ...
                if (file_exists($u = USER . DS . $key . '.page')) {
                    ...
                    if (strpos($secret, X) === 0) {
                        if (password_verify($pass . ' ' . $key, substr($secret, 1))) {
                            $enter = true;
                        }
                    // Validate password!
                    } else {
                    ...
                    if ($enter) {
                        ...
                        Guardian::kick(isset($r['kick']) ? $r['kick'] : $url->previous);
                    } else {
